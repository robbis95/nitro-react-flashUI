...
{
    const { list = null, selectedFriendsIds = null, selectFriend = null, setShowHoverText = null } = props;

    if(!list || !list.length) return null;

    return (
        <>
            { list.map((item, index) => <FriendsListGroupItemView key={ index } friend={ item } selected={ selectedFriendsIds && (selectedFriendsIds.indexOf(item.id) >= 0) } selectFriend={ selectFriend } setShowHoverText={ (e) => setShowHoverText(e) } />) }
        </>  
    );
}
