...
export const NavigatorView: FC<{}> = (props) => {
    ...
    const [categories, setCategories] =
        useState<NavigatorCategoryDataParser[]>(null);
    const [topLevelContext, setTopLevelContext] =
        useState<NavigatorTopLevelContext>(null);
    const [topLevelContexts, setTopLevelContexts] =
        useState<NavigatorTopLevelContext[]>(null);
    const [navigatorData, setNavigatorData] = useSharedState<NavigatorData>(
        "@navigatorData",
        {
    ...
    const [searchResult, setSearchResult] =
        useState<NavigatorSearchResultSet>(null);
    const [pendingDoorState, setPendingDoorState] =
        useState<{ roomData: RoomDataParser; state: string }>(null);
    const pendingSearch = useRef<{ value: string; code: string }>(null);

    const onUpdateDoorStateEvent = useCallback(
        (event: UpdateDoorStateEvent) => {
            switch (event.type) {
                ...
                case UpdateDoorStateEvent.STATE_NO_ANSWER:
                    setPendingDoorState((prevValue) => {
                        if (
                            prevValue.state ===
                            UpdateDoorStateEvent.STATE_WAITING
                        )
                            GoToDesktop();
    ...
    UseUiEvent(
        UpdateDoorStateEvent.STATE_WRONG_PASSWORD,
        onUpdateDoorStateEvent
    );
    ...
    const sendSearch = useCallback(
        (searchValue: string, contextCode: string) => {
            setCreatorOpen(false);
    ...
    const getRoomDoorState = useMemo(() => {
        ...
        switch (pendingDoorState.state) {
            ...
            case UpdateDoorStateEvent.STATE_NO_ANSWER:
                return (
                    <NavigatorRoomDoorbellView
                        roomData={pendingDoorState.roomData}
                        state={pendingDoorState.state}
                        onClose={closePendingDoorState}
                    />
                );
            case UpdateDoorStateEvent.START_PASSWORD:
            case UpdateDoorStateEvent.STATE_WRONG_PASSWORD:
                return (
                    <NavigatorRoomPasswordView
                        roomData={pendingDoorState.roomData}
                        state={pendingDoorState.state}
                        onClose={closePendingDoorState}
                    />
